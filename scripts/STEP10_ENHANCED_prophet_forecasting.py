...
capacity_analysis.to_csv('../results/STEP10_ENHANCED_capacity_planning.csv', index=False)

# Save detailed metrics
enrolment_df.to_csv('../results/STEP10_ENHANCED_enrolment_metrics.csv', index=False)
biometric_df.to_csv('../results/STEP10_ENHANCED_biometric_metrics.csv', index=False)
demographic_df.to_csv('../results/STEP10_ENHANCED_demographic_metrics.csv', index=False)
...
for state in top_5_demand:
    if state in enrolment_forecasts:
        enrolment_forecasts[state].to_csv(
            f'../results/STEP10_ENHANCED_enrolment_forecast_{state.replace(" ", "_").lower()}.csv', 
            index=False
        )
    if state in biometric_forecasts:
        biometric_forecasts[state].to_csv(
            f'../results/STEP10_ENHANCED_biometric_forecast_{state.replace(" ", "_").lower()}.csv', 
            index=False
        )
    if state in demographic_forecasts:
        demographic_forecasts[state].to_csv(
            f'../results/STEP10_ENHANCED_demographic_forecast_{state.replace(" ", "_").lower()}.csv', 
            index=False
        )

# Save models for visualization
import pickle
with open('../results/STEP10_ENHANCED_models.pkl', 'wb') as f:
    pickle.dump({
